...
This version is meant to be run once directly, with all hyperparameters editedat the top of the file instead of using sweeps or a config object.
...
The only substantive change is that the set aggregation is done with explicitmasked mean/max pooling rather than the original sum-based DeepSet block."""
...
class S2EventDataset(Dataset):def init(self, X: np.ndarray, Y: np.ndarray, event_groups: List[np.ndarray]):self.X = Xself.Y = Yself.event_groups = event_groups
...
def collate_events(batch: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:batch_size = len(batch)max_clusters = max(item["n_clusters"] for item in batch)
...
def compute_soft_metrics(y_true: np.ndarray, y_prob: np.ndarray) -> Dict[str, float]:metrics: Dict[str, float] = {}metrics["mse"] = float(((y_prob - y_true) ** 2).mean())
...
def run_epoch(model: nn.Module,loader: DataLoader,optimizer: Optional[torch.optim.Optimizer] = None,collect_metrics: bool = False,) -> Tuple[float, Dict[str, float]]:is_train = optimizer is not Nonemodel.train() if is_train else model.eval()

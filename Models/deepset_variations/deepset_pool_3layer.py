...
class MaskedPoolingNet(nn.Module):
    """Per-cluster encoder + masked mean/max pooling + deeper per-cluster head."""
    ...
    def __init__(
        ...
        latent_dim: int = 128,
        phi_hidden: int = 512,
        phi_hidden_2: int = 512,
        head_hidden: int = 512,
        head_hidden_2: int = 256,
        output_dim: int = 2,
    ):
        super().__init__()

        # Deeper encoder: 5 -> phi_hidden -> phi_hidden_2 -> latent_dim
        self.encoder = nn.Sequential(
            nn.Linear(input_dim, phi_hidden),
            nn.ReLU(),
            nn.Linear(phi_hidden, phi_hidden_2),
            nn.ReLU(),
            nn.Linear(phi_hidden_2, latent_dim),
            nn.ReLU(),
        )

        # Deeper head: (latent_dim * 3) -> head_hidden -> head_hidden_2 -> output_dim
        self.head = nn.Sequential(
            nn.Linear(latent_dim * 3, head_hidden),
            nn.ReLU(),
            nn.Linear(head_hidden, head_hidden_2),
            nn.ReLU(),
            nn.Linear(head_hidden_2, output_dim),
        )

...
def collect_main_event_diagnostics(
    ...
    """
    Build one row per event, comparing events where the model correctly
    identifies the p_main cluster vs events where it misses.
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for b in range(x_cpu.shape[0]):
                ...
                xb = x_cpu[b, valid]       # [n_clusters, n_features]
                ...
                # Top-2 prediction margin: useful for separating confident vs uncertain misses.
                if n >= 2:
                    pred_top2 = torch.topk(pred_main, k=2).values
                    true_top2 = torch.topk(true_main, k=2).values
                    pred_main_margin = float(pred_top2[0] - pred_top2[1])
                    true_main_margin = float(true_top2[0] - true_top2[1])
                else:
                ...
                # Event-level summaries of standardized features.
                for j, feature_name in enumerate(FEATURES):
                    vals = xb[:, j]
                    row[f"{feature_name}_mean"] = float(vals.mean())
                    row[f"{feature_name}_std"] = float(vals.std(unbiased=False))
                    row[f"{feature_name}_min"] = float(vals.min())
                    row[f"{feature_name}_max"] = float(vals.max())

                    row[f"true_main_{feature_name}"] = float(xb[true_main_idx, j])
                    row[f"pred_main_{feature_name}"] = float(xb[pred_main_idx, j])
                    row[f"pred_minus_true_main_{feature_name}"] = float(
...
def summarize_main_diagnostics(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for feature_name in FEATURES:
        summary_cols.extend(
            [
                ...
                f"{feature_name}_min",
                f"{feature_name}_max",
                f"true_main_{feature_name}",
                f"pred_main_{feature_name}",
                f"pred_minus_true_main_{feature_name}",
            ]
        )
    ...
    multiplicity_summary = (
        ...
        .agg(
            ...
            mean_pred_margin=("pred_main_margin", "mean"),
            median_pred_margin=("pred_main_margin", "median"),
...
def train() -> Dict[str, float]:
    ...
    results = {
        ...
        # Event-level diagnostics on validation and test splits.

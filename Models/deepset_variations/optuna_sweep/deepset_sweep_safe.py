...
- graph-level pooling with global_add_pool/global_mean_pool/sum+mean, selected by Optuna
...
class GraphSetTagger(nn.Module):
    """Per-cluster encoder + configurable graph pooling + per-cluster head."""
    ...
    def __init__(
        ...
        if pooling not in {"sum", "mean", "sum_mean"}:
            raise ValueError(f"Unknown pooling type: {pooling}")
        self.pooling = pooling
        graph_context_dim = latent_dim * 2 if pooling == "sum_mean" else latent_dim
        ...
        self.head = build_mlp(
            input_dim=latent_dim + graph_context_dim,
    ...
    def forward(self, batch: PYGData) -> torch.Tensor:
        ...
        if self.pooling == "sum":
            ...
            graph_embed = global_mean_pool(node_embed, graph_index)
        elif self.pooling == "sum_mean":
            graph_sum = global_add_pool(node_embed, graph_index)
            graph_mean = global_mean_pool(node_embed, graph_index)
            graph_embed = torch.cat([graph_sum, graph_mean], dim=-1)
        else:
...
def bce_loss(
    ...
    p_alt_loss_weight: float = 1.0,
) -> torch.Tensor:
    """BCE loss with a tunable weight on p_alt.

    p_main is always trained with weight 1.0. p_alt can be downweighted or
    disabled when the sweep should prioritize p_main calibration/ranking.
    p_alt_loss_weight=1.0 reproduces the original equal-weight BCE behavior.
    """

    p_main_loss = nn.functional.binary_cross_entropy_with_logits(
        logits[:, 0],
        targets[:, 0],
    )
    ...
    return p_main_loss + p_alt_loss_weight * p_alt_loss
...
def bce_loss_with_event_sum_penalty(
    ...
    bce = bce_loss(logits, targets, p_alt_loss_weight=p_alt_loss_weight)
...
def suggest_trial_params(trial: optuna.Trial) -> Dict[str, Any]:
    ...
    params: Dict[str, Any] = {
        ...
        "pooling": trial.suggest_categorical("pooling", ["sum", "mean", "sum_mean"]),
        ...
        "p_alt_loss_weight": trial.suggest_categorical(
            "p_alt_loss_weight",
...
def evaluate_best_trial(study: optuna.Study) -> Dict[str, float]:
    ...
    test_loss, test_metrics = run_epoch(
        ...
        p_alt_loss_weight=float(params.get("p_alt_loss_weight", 1.0)),
    )

"""MSE experiment: fraction-enriched event batches plus sum/mean/max pooling.

The loss remains ordinary per-cluster MSE. Only training-event sampling changes:
an event with at least one fractional p_main target is sampled more often. This
preserves complete event context and leaves the validation distribution and all
reported validation metrics unweighted. Test events are never evaluated here.
"""
...
OUTPUT_ROOT = Path(__file__).resolve().parent / "fraction_enriched_pooling_output"
...
class SumMeanMaxDeepSetRegressor(nn.Module):
    ...
    def forward(self, batch) -> torch.Tensor:
        ...
        return self.head(torch.cat((embedding, event_context), dim=-1))


def event_fractional_mask(groups: List[np.ndarray], y: np.ndarray) -> np.ndarray:
    return np.asarray([
        np.any((y[rows, 0] > FRACTIONAL_LOW) & (y[rows, 0] < FRACTIONAL_HIGH))
        for rows in groups
    ], dtype=bool)
...
def main() -> None:
    ...
    parser.add_argument(
        "--fractional-event-weight", type=float, default=4.0,
        help="Relative probability for a training event containing >=1 fractional label (1.0 disables enrichment).",
    )
    ...
    train_dataset = EventDataset(x_train, y_train, train_groups)
    val_dataset = EventDataset(x_val, y_val, val_groups)
    ...
    if not args.no_wandb and wandb is not None:
        run = wandb.init(project=WANDB_PROJECT, name=f"sum-mean-max-fracweight{args.fractional_event_weight:g}-seed{args.seed}", config={
            "experiment": "fraction_enriched_event_sampling_sum_mean_max", "feature_set": FEATURE_SET,
            "features": features, "target": "p_main", "loss": "mse", "pooling": "sum_mean_max",
            ...
            "test_policy": "not evaluated during selection", "gradient_clip_norm": GRADIENT_CLIP_NORM,
            "learning_rate": LEARNING_RATE, "weight_decay": WEIGHT_DECAY, "batch_size": BATCH_SIZE, **cut_stats,
        })
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        if val["p_main_mse"] < best_val_mse:
            ...
            torch.save({"epoch": epoch, "model_state_dict": model.state_dict(), "best_val_p_main_mse": best_val_mse, "features": features, "seed": args.seed, "fractional_event_weight": args.fractional_event_weight, "cut_stats": cut_stats}, checkpoint_path)
        else:
            stale_epochs += 1
        metrics = {
            "epoch": epoch, "train_p_main_mse": train_mse, "val_p_main_mse": val["p_main_mse"],
        ...
        if run:
            wandb.log(metrics)
        print(f"epoch={epoch:03d} train_mse={train_mse:.6f} val_mse={val['p_main_mse']:.6f} val_frac={val['fractional_mse']:.6f} val_r2={val['p_main_r2']:.5f} best={best_val_mse:.6f}")
        if stale_epochs >= EARLY_STOPPING_PATIENCE:
            break
    summary = {
        "feature_set": FEATURE_SET, "pooling": "sum_mean_max", "seed": args.seed,
        "fractional_event_weight": args.fractional_event_weight, "raw_fractional_event_rate": raw_fractional_event_rate,
        "expected_sampled_fractional_event_rate": sampled_fractional_event_rate,
        "best_epoch": best_epoch, "best_val_p_main_mse": best_val_mse,
        "best_validation_checkpoint": str(checkpoint_path), "held_out_test_evaluated": False, **cut_stats,
    }
